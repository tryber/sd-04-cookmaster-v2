...
const insertIMG = async (id, url) => {
  await connection().then((db) =>
    db.collection('recipes').updateOne({ _id: ObjectId(id) }, { $set: { image: url } }),
  );
};

module.exports = { addRecipe, showRecipes, showRecipeByid, editRecipe, deleteRecipe, insertIMG };
