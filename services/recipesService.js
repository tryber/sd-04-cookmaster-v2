...
const deleteAdmOrUser = async (id, user) => {
  const { role, _id } = user;
  if (role === 'admin') return modelDelet(id);
  const { userId } = await recipeModel.findById('recipes', id);
  console.log(userId);
  if (String(userId) === String(_id)) return modelDelet(id);
  return null;
};
